...
您好，我是王鹏程；现在正在四川大学就读研究生二年级;主要的研究方向是数字图像处理和计算机视觉；研究生阶段主要使用语言是c++，之前主要的项目是老师所带领的一个校园无人公交车的项目；我主要在里面负责车辆监控客户端和摄像头图像传输服务器，以及实时的深度学习目标检测网络;所有软件代码在Nvidia Jetson TX2 上的集成和整合。现在主要是在实现深度学习目标检测网络的推理加速和异构计算设备的支持。